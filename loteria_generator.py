...
if st.button("🎰 Generar combinaciones"):
    ...
    for i in range(cantidad_jugadas):
        st.caption("🎵 Sonido de ruleta activado")
        components.html(f"""
            <audio autoplay id='sound-{str(uuid.uuid4())}'>
                <source src='https://files.freemusicarchive.org/storage-freemusicarchive-org/music/no_curator/Monplaisir/Casino_Music/Monplaisir_-_Roulette.mp3' type='audio/mpeg'>
        ...
        components.html(f"""
            <audio autoplay id='audio-{{unique_id}}'>
                <source src='https://cdn.pixabay.com/download/audio/2022/03/15/audio_a7106f1984.mp3?filename=interface-124464.mp3' type='audio/mpeg'>
            </audio>
        """, height=0)
        components.html("""
        <audio autoplay>
